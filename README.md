...
![AI: Gemini](https://img.shields.io/badge/AI-Gemini%202.0-1A73E8?logo=google)
...
A two-platform VR educational system for teaching PC hardware and assembly, built as a final-year engineering project (PFE) during an internship at UPTAKERS. Students learn through a Unity-based VR game on Meta Quest while professors monitor live progress on a Next.js web dashboard, with the two halves synchronizing over a signed HTTP API. The standout module ends with an AI-graded build review — the student's PC layout is validated against eleven design checks and Google Gemini generates dynamic bilingual feedback.
...
| Email | Nodemailer 8 |
| AI feedback | Google Gemini (`gemini-2.0-flash`) via REST |
